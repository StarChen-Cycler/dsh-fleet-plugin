...
- **浏览器实测（用户截图 17:36）**：门户卡片 Windows-PC 在线，CPU 7.3%、内存
  31.7/63.9GB（50%）、磁盘 447.9/499.2GB（90%）、16 核——与任务管理器一致（±5% 内）。
- vps-node（VPS 自身，Linux 自启测试节点）显示离线：该机未运行 DSH（3080 无服务），
  探测失败 → 变灰，属预期行为（节点探测式在线的正确表现）。
